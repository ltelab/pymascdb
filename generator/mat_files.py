...
def lat_lon_alt(campaign):
    ...
    if campaign in {"APRES3-2016", "APRES3-2017"}:
        lat = -66.6628
        lon = 140.0014
        alt = 41.0
        ...
        lon = 128.6996
        alt = 789.0
    elif campaign in {"Jura-2019", "Jura-2023"}:  # Les Charbonnieres
        lat = 46.6702
        lon = 6.3125
        alt = 1045.0
...
def pad_with_zeros(A, r=1024, c=1024):
    ...
    if (ll >= 0) and (bb >= 0):
        ...
        return out
    if (ll < 0) and (bb > 0):  # crop on the right (arbitrary choice)
        out[:, bb : (bb + c_)] = A[0:r, :]
        return out
    if ll > 0:  # crop on the top
        out[ll : (ll + r_), :] = A[:, 0:c]
        return out
...
def masc_mat_file_to_dict(fn, pix_size=33.5e-6):
    ...
    dict = {
        ...
        "ell_fit_area": (
            mat["E"]["a"][0, 0][0, 0] * mat["E"]["b"][0, 0][0, 0] * np.pi * p2
        ),  # [m**2]                      10. -
        "ell_fit_ori": mat["E"]["theta"][0, 0][0, 0],  # Fitted ell orientation [°]                            11. -
        "ell_fit_a_r": (
            mat["E"]["b"][0, 0][0, 0] / mat["E"]["a"][0, 0][0, 0]
        ),  # Fitted ell asp. ratio                12. III
        "ell_fit_ecc": np.sqrt(
            1 - mat["E"]["b"][0, 0][0, 0] / mat["E"]["a"][0, 0][0, 0],
        ),  # Fitted ell eccentric.          13. I
        ...
        "ell_in_area": (
            mat["E_in"]["a"][0, 0][0, 0] * mat["E_in"]["b"][0, 0][0, 0] * np.pi * p2
        ),  # [m**2]                17. I
        "ell_out_A": (
            mat["E_out"]["a"][0, 0][0, 0] * p1
        ),  # Outer ell maj dim [m] --same center as fitted one, orientation as inner one  18. -
        "ell_out_B": (
            mat["E_out"]["b"][0, 0][0, 0] * p1
        ),  # Outer ell min dim [m] --same center as fitted one, orientation as inner      19. I, II, III
        "ell_out_area": (
            mat["E_out"]["a"][0, 0][0, 0] * mat["E_out"]["b"][0, 0][0, 0] * np.pi * p2
        ),  # [m**2]                       20. -
        # C3: particle shape
        "roundness": mat["roundness"][0][0],  # area / circum. circle ratio [-]                     30. III
        "p_circ_out_r": (
            mat["perim"][0][0] / (2.0 * np.pi * mat["C_out"]["r"][0, 0][0, 0])
        ),  # perim /circum p ratio [-]          31. II, III
        "rectangularity": mat["Rect"]["A_ratio"][0, 0][
            0,
            0,
        ],  # Area to bounding box area ratio [-]                 32. -
        "bbox_width": (
            mat["Rect"]["width"][0, 0][0, 0] * p1
        ),  # Bound. box width [m]                                33. -
        "bbox_len": mat["Rect"]["length"][0, 0][0, 0] * p1,  # Bound. box length [m]                               34. -
        "rect_perim_ratio": mat["Rect"]["p_ratio"][0, 0][
            0,
            0,
        ],  # Box perimeter to particule perim ratio [-]            35. I, II, III
        "rect_aspect_ratio": mat["Rect"]["aspect_ratio"][0, 0][
            0,
            0,
        ],  # Bounding box aspect ratio [-]                         36. I
        "rect_eccentricity": mat["Rect"]["eccentricity"][0, 0][
            0,
            0,
        ],  # Bounding bo eccentricity [-]                          37. -
        "solidity": mat["hull"]["solidity"][0, 0][0, 0],  # area / CH area ratio    [-]                           38. I
        "convexity": mat["hull"]["convexity"][0, 0][
            0,
            0,
        ],  # perim / CH perim ratio  [-]                           39. I, III
        "hull_n_angles": (
            mat["hull"]["xh"][0, 0].shape[0]
        ),  # number of CH vertices   [-]                           40. II, III
        "p_circ_r": (
            mat["perim"][0][0] / (2.0 * np.pi * mat["eq_radius"][0][0])
        ),  # perim /eq. circ p ratio [-]         41. III
        ...
        "skel_N_ends": mat["skel"]["N_ends"][0, 0][
            0,
            0,
        ],  # Number of skeleton ends [-]                                 44. III
        "skel_N_junc": mat["skel"]["N_junctions"][0, 0][
            0,
            0,
        ],  # Number of skeleton junctions [-]                            45. -
        "skel_perim_ratio": mat["skel"]["p_ratio"][0, 0][
            0,
            0,
        ],  # Skeleton length to perimeter ratio [-]                      46. -
        "skel_area_ratio": mat["skel"]["A_ratio"][0, 0][
            0,
            0,
        ],  # Skeleton length to area ratio [pix**-1]                     47. -
        # C5: rotational symmetry
        #'sym_P0':       mat['Sym']['P0'][0,0][0,0],                   # Standardized distance to centroid Fourier power spectrum comp. P0 48. -
        "sym_P1": mat["Sym"]["P1"][0, 0][
            0,
            0,
        ],  # Standardized distance to centroid Fourier power spectrum comp. P1 49. I
        "sym_P2": mat["Sym"]["P2"][0, 0][
            0,
            0,
        ],  # Standardized distance to centroid Fourier power spectrum comp. P2 50. II
        "sym_P3": mat["Sym"]["P3"][0, 0][
            0,
            0,
        ],  # Standardized distance to centroid Fourier power spectrum comp. P3 51. I, II, III
        "sym_P4": mat["Sym"]["P4"][0, 0][
            0,
            0,
        ],  # Standardized distance to centroid Fourier power spectrum comp. P4 52. II, III
        "sym_P5": mat["Sym"]["P5"][0, 0][
            0,
            0,
        ],  # Standardized distance to centroid Fourier power spectrum comp. P5 53. -
        "sym_P6": mat["Sym"]["P6"][0, 0][
            0,
            0,
        ],  # Standardized distance to centroid Fourier power spectrum comp. P6 54. I, II
        "sym_Pmax_id": pmax06(mat["Sym"])[
            0
        ],  # Max ID among P0 to P6                                             55. I, III
        "sym_P6_max_ratio": (
            mat["Sym"]["P6"][0, 0][0, 0] / pmax06(mat["Sym"])[1]
        ),  #                                                            56. II
        "sym_mean": mat["Sym"]["mean"][0, 0][
            0,
            0,
        ],  # Mean distance to centroid [pix] ?                                 57.
        "sym_std": mat["Sym"]["std"][0, 0][
            0,
            0,
        ],  # STD distance to centroid  [pix] ?                                 58. I, II
        "sym_std_mean_ratio": (
            mat["Sym"]["std"][0, 0][0, 0] / mat["Sym"]["mean"][0, 0][0, 0]
        ),  #                    59. I, II
        # C6: texture operators
        "intensity_mean": mat["mean_intens"][0][
            0
        ],  # Mean intensity (not sure about units, seems normalized)        60. I, II, III
        "intensity_max": mat["max_intens"][0][
            0
        ],  # Max intensity /brightness                                      61. II, III
        "contrast": mat["contrast"][0][0],  # contrast                                                       62. I
        "intensity_std": mat["std"][0][0],  # std intensity                                                  63. III
        "hist_entropy": mat["hist_entropy"][0][
            0
        ],  # Brightness histogram entropy                                   64. -
        "local_std": mat["local_std"][0][
            0
        ],  # Average grey-level Local standard deviation 3x3                65. I, II, III
        "local_intens": mat["range_intens"][0][
            0
        ],  # Local average range intensity (probably 3x3 window)            66. I, III
        ...
        "snowflake_class_prob": round(mat["label_probs"][0][mat["label_ID"][0][0] - 1], 2),
    }  # Prob of label X
...
def masc_mat_triplet_to_dict(fnames, pix_size=33.5e-6, campaign=""):
    ...
    # Melting
    melting_prob = (mat0["melting_probs"][0] + mat1["melting_probs"][0] + mat2["melting_probs"][0]) / 3.0
    ...
    dict = {
        ...
        "flake_n_roi": np.mean(
            [mat0["n_roi"][0][0], mat1["n_roi"][0][0], mat2["n_roi"][0][0]],
        ),  # Avg # of particles per cam [-]
        # Dmax
        "flake_Dmax": Dmax,  # m
        # Riming degree
...
def triplet_images_reshape(fnames, pix_size=33.5e-6, newshape=None):
    """
